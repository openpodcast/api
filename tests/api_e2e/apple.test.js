...
const appleEpisodeDetailsPayloadNoFollowState = require('../../fixtures/appleEpisodeDetailsNoFollowState.json')
...
describe('check Connector API with appleEpisodeDetailsNoFollowState', () => {
    it('should return status 200 when sending proper Apple payload (but no follow state data)', async () => {
        const response = await request(baseURL)
            .post('/connector')
            .set(auth)
            .send(appleEpisodeDetailsPayloadNoFollowState)
        expect(response.statusCode).toBe(200)
    })
})
