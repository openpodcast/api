...
const authMiddleware = function (req: Request, res: Response, next: Function): any {
  const authToken: String | undefined = Array.isArray(req.headers.authorization) ? req.headers.authorization[0] : req.headers.authorization

  // TODO: for now just check if there is a Bearer consisting of a few chars
  if (authToken === undefined || authToken.length < 7 || !authToken.startsWith('Bearer ')) {
