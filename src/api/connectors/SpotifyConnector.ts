...
class SpotifyConnector implements ConnectorHandler {
  async handleDetailedStreams (accountId: number, payload: { detailedStreams: Object[] } | undefined): Promise<void> | never {
    // just a quick and dirty approach to implement one api

    if (payload === undefined || payload.detailedStreams === undefined || !Array.isArray(payload.detailedStreams)) {
    ...
    const insertStmt = 'INSERT INTO spotifyDetailedStreams (account_id, sps_date, sps_starts, sps_streams) VALUES (?,?,?,?)'

    await Promise.all(
      payload.detailedStreams
        .map(async (entry: any): Promise<any> => await connection.execute(insertStmt, [accountId, entry.date, entry.starts, entry.streams]))
    )

    return connection.unprepare(insertStmt)
  }

  async handleRequest (accountId: number, payload: JsonPayload): Promise<void> | never {
